...
def main():
    ...
    paradigm_types = ('_V1_','_V2_','_V3_','_V4_','_V5_','_V6_')
    ##import fwd
                            ...
                            if paradigm_type in file_name:
                                try:
                                    ...
                                    for epo,band,graph in zip(epochs,bands,graphs):
                                        RawData().save_epoch(epo,restage_dir,file_dir,file_name.replace(paradigm_type,save_id),band,
                                                fmin=1,fmax=45,graphs=graph)
                                    
                                    evoked = epoch.average()
                                    fig1 = evoked.plot_topomap(times=(range(tmax)))
                                    fig1.savefig(file_name.replace(paradigm_type,save_id).replace('MIPDB_raw',data_type).replace('raw.fif','topo.png'),overwrite=True)
                                except:
                                    continue
...
event_id={'start_of_video3':83} 
save_id = '_V3_'
tmin=-0.2
tmax=20
data_type = 'MIPDB_video_3'
method='dSPM'
